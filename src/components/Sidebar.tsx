import { LayoutDashboard, Users, Truck, DollarSign, FileText, Settings, LogOut, Fuel, BookOpen, CarFront, Sun, Moon, MapPin, ClipboardList, BotMessageSquare, Box, PanelLeftClose, X } from 'lucide-react';
...
export default function Sidebar({ currentPage, onNavigate, onClose, isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside className="h-full w-64 border-r border-theme-border flex flex-col bg-[var(--theme-sidebar-bg,#070914)] text-theme-text transition-all duration-300 relative">
      <div className="px-5 py-5 flex items-center justify-between border-b border-theme-border/50 mb-2 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 bg-gradient-primary rounded-xl flex items-center justify-center shadow-[0_0_25px_rgba(139,61,255,0.35)] transition-all duration-300 hover:scale-105 shrink-0">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-semibold rounded-xl sidebar-item cursor-pointer",
                isActive 
                  ? item.id === 'payments'
                    ? "sidebar-item-active-green font-bold"
            ...
            >
              <Icon className={cn("w-5 h-5 shrink-0 transition-all duration-300", isActive ? item.id === 'payments' ? "text-emerald-400 stroke-emerald-400" : "" : item.id === 'fuelsuite' ? "" : "")} />
              <span className="truncate">{item.label}</span>
              {item.id === 'fuelsuite' && (
                <span className="ml-auto bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:shadow-[0_0_15px_rgba(59,130,246,0.15)] text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm shrink-0">NEW</span>
              )}
            </button>
          )
        })}
      ...
      <div className="p-4 border-t border-theme-border mt-auto shrink-0">
        <div className="mb-3 space-y-1.5">
          <button
            ...
            className={cn(
              "w-full flex items-center gap-3 px-3.5 py-2 text-sm font-semibold rounded-xl sidebar-item cursor-pointer",
              currentPage === 'assistant' 
                ? "sidebar-item-active font-bold" 
                : "text-theme-text-muted hover:bg-white/5 hover:text-theme-text"
            )}
          >
            <BotMessageSquare className="w-5 h-5 shrink-0 transition-all duration-300" />
          ...
          >
            <Settings className="w-5 h-5 shrink-0 transition-all duration-300" />
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 mb-3 bg-theme-panel border border-theme-border rounded-xl shadow-[0_0_15px_rgba(139,61,255,0.05)]">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#8B3DFF] to-[#3B82F6] flex items-center justify-center font-bold text-sm text-white shadow-[0_0_15px_rgba(139,61,255,0.3)] shrink-0">
